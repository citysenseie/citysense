...
import { createLiveLocationNotification } from "../services/notificationService";
...
const confirmSharing = async () => {
  ...
  (contact) =>
    contact.id && selectedContactIds.includes(contact.id)
);

const linkedRecipientUserIds = selectedContacts
...
if (!currentUser) {
  throw new Error("You must be signed in to share your location.");
}

await Promise.all(
  linkedRecipientUserIds.map((recipientUserId) =>
    createLiveLocationNotification({
      recipientUserId,
